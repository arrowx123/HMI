...
public class GameController : MonoBehaviour
{
    ...
    private bool receive_zero = true;
    ...
    public bool enable_collision_sound = false;

    //	experiment parameters
    public bool randomize_order = true;
    //1: only video
    //2: video + vibrotactile
    //3: video + vibrotactile + force
    ...
    private float[] workspaceUpdateValue = new float[1];

    public bool get_enable_collision_sound() {
        return enable_collision_sound;
    }

    public bool get_receivePositionData()
    {
        return receivePositionData;
    }

    public bool get_receive_zero()
    {
        return receive_zero;
    }
    ...
    void Start()
    {
        ...
        //OptiTrackManager.Instance.set_receive_zero(receive_zero);
    ...
    IEnumerator startVibration()
    {
        ...
        Debug.Log("startVibration coroutine!");
        calledStartVibration = false;
        //		print (Time.time);
    }
...
/*		    && drillBitSpeed != 0)) {) {*/)
        ...
        if (output_warning)
        {
            if (play_mode != 1 && OSCDataReceiverController.getTriggerControl() == true)
                OSCDataSenderController.send_maximum_torque_message();
            else
                OSCDataSenderController.send_stop_message();
