...
public class GenericFunctionsClass : MonoBehaviour
{
    ...
    private float error_thres = 2.0f / 20.0f;
    ...
    private float[] position_y = { 0.0f, -0.25f, 0.0f, -0.25f, 0.0f, -0.25f, 0.0f, -0.25f };
    private float[] position_z = { 0f, 0f, -0.25f, -0.25f, -0.5f, -0.5f, -0.75f, -0.75f };
    private float x_offset = 3.8f;

    //private string[] force_feedback_tags = { "other_collider",
    //    "bolt_0", "bolt_1", "bolt_2", "bolt_3",
    //    "bolt_4", "bolt_5", "bolt_6", "bolt_7",
    //    "flangenut_0", "flangenut_1", "flangenut_2", "flangenut_3",
    //    "flangenut_4", "flangenut_5", "flangenut_6", "flangenut_7" };

    private string[] force_feedback_tags = { "other_collider",
        "flangenut_s_0", "flangenut_s_1", "flangenut_s_2", "flangenut_s_3",
        "flangenut_s_4", "flangenut_s_5", "flangenut_s_6", "flangenut_s_7" };
    ...
    void Awake()
    {
        ...
        if (GameControllerObject != null)
            ...
            Debug.Log("GenericFunctionsClass: can not find GameController");
    }
    ...
    public void SetHapticWorkSpace()
    {
        ...
        Debug.Log("SetHapticWorkSpace: ");
        Debug.Log(myHapticClassScript.myWorkSpaceSize);
    }
    ...
    public void SetTwoHapticWorkSpaces()
    {
        ...
        Debug.Log("myFloatPosition: " + myFloatPosition);
    ...
    //public void initialize_proxy_values() {
    ...
    //    handleOriginalRot = get_orientation();
    //    handleOriginalPos = get_position();
    //}

    private Quaternion get_orientation()
    {
        ...
        while (true)
        {
            ...
            Quaternion rot = new Quaternion(OrientationCursor.x, OrientationCursor.y, OrientationCursor.z, OrientationCursor.w);
            Vector3 euler = rot.eulerAngles;
            rot.eulerAngles = new Vector3(euler.z, euler.y, euler.x);

            //if (rot.eulerAngles != new Vector3(0f, 90f, 0f))
            //{
            Debug.Log("rot:");
            Debug.Log(rot);
    ...
    private void process_position()
    {
        ...
        if (subtask_index >= position_y.Length || subtask_index < 0)
            return;
        ...
        if (wrenchFittingController.isCoupledSpecific() == subtask_index)
        {
            ...
            if (error < error_thres)
            {
                ...
                Vector3 newPos = new Vector3(newX + x_offset, position_y[subtask_index], position_z[subtask_index]);
    ...
    public void GetProxyValues()
    {
        ...
        //handleLastPos = handleCurrentPos;
        //handleCurrentPos = get_position();
        //Vector3 diff_handle = (handleCurrentPos - handleOriginalPos);
        ////myHapticClassScript.hapticCursor.transform.position = diff_handle;
        //myHapticClassScript.hapticCursor.transform.position = get_position();
        ...
        //myHapticClassScript.hapticCursor.transform.rotation = Quaternion.Euler(new Vector3( euler_handle.z, euler_handle.x, euler_handle.y));
        //myHapticClassScript.hapticCursor.transform.rotation = Quaternion.Euler(new Vector3(euler_handle.x, euler_handle.y, euler_handle.z));

        //myHapticClassScript.hapticCursor.transform.localRotation = Quaternion.Euler(new Vector3(euler_handle.x, euler_handle.y, euler_handle.z));
        //myHapticClassScript.hapticCursor.transform.localRotation = Quaternion.Euler(new Vector3(0f, 0f, 0f));
        //Quaternion quater = get_orientation();
        //myHapticClassScript.hapticCursor.transform.localRotation = Quaternion.Euler(quater.eulerAngles.z, -quater.eulerAngles.y, quater.eulerAngles.x);
        ...
        //Vector4 OrientationCursor = new Vector4();
        //OrientationCursor = ConverterClass.ConvertDouble4ToVector4(myProxyOrientation);
        //      Quaternion OrientationCursor_Quater = new Quaternion(OrientationCursor.x, OrientationCursor.y, OrientationCursor.z, OrientationCursor.w);
        //      Vector3 OrientationCursor_angle = OrientationCursor_Quater.eulerAngles;


        //      Vector3 modified_angle = new Vector3(OrientationCursor_angle.x, OrientationCursor_angle.y, OrientationCursor_angle.z);

        //      //Assign Haptic Values to Cursor
        //      //myHapticClassScript.hapticCursor.transform.localEulerAngles = Quaternion.Euler(modified_angle);
        //      myHapticClassScript.hapticCursor.transform.localRotation = Quaternion.Euler(modified_angle);
        //      //myHapticClassScript.hapticCursor.transform.rotation = Quaternion.Euler(modified_angle);
        //      //myHapticClassScript.hapticCursor.transform.Rotate
        //      //Debug.Log("positionCursor:");
        //      //Debug.Log(positionCursor);

        //      Debug.Log("OrientationCursor_Quater:");
        //      Debug.Log(OrientationCursor_Quater.eulerAngles);
        //      Debug.Log("myHapticClassScript.hapticCursor.transform.rotation: ");
        //      Debug.Log(myHapticClassScript.hapticCursor.transform.rotation.eulerAngles);
    }
    ...
    public void SetHapticGeometry()
    {
        ...
        for (int i = 0; i < force_feedback_tags.Length; i++)
        {
            GameObject[] tmp_objects = GameObject.FindGameObjectsWithTag(force_feedback_tags[i]) as GameObject[];
            List<GameObject> tmp_list = new List<GameObject>(tmp_objects);
            objects.AddRange(tmp_list);
        }
        ...
        {
            /***************************************************************/
            //Debug.Log("myObjects[i].name: " + myObjects[ObjId].tag);
